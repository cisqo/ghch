...
async function importAllIssues(syncInterval) {
  const query = join(['state:open', createDateFilter(syncInterval)], ' ');

  await Promise.all(
    map(
      projectMapping,
      async (project, repo) => await importIssuesFromRepo({ repo, project, query }),
    ),
  );
...
importAllIssues(process.env.SYNC_INTERVAL).catch((e) => {
  console.log(e);
  process.exit(1);
});
